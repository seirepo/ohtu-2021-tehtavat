...
class TestOstoskori(unittest.TestCase):
    ...
    def test_kahden_saman_tuotteen_lisaamisen_jalkeen_ostosolion_nimi_ja_maara_ok(self):
        mehu = Tuote("mehu", 3)
        self.kori.lisaa_tuote(mehu)
        self.kori.lisaa_tuote(mehu)
    ...
    def test_korin_ostosolion_lukumaara_paivittyy_jos_kahdesta_samasta_tuotteesta_toinen_poistetaan(self):
        mehu = Tuote("mehu", 3)
        self.kori.lisaa_tuote(mehu)
        self.kori.lisaa_tuote(mehu)
        self.kori.poista_tuote(mehu)

        ostos = self.kori.ostokset()[0]

        self.assertEqual(ostos.lukumaara(), 1)

    def test_kori_on_tyhja_jos_ainoa_koriin_lisatty_tuote_poistetaan(self):
        mehu = Tuote("mehu", 3)
        self.kori.lisaa_tuote(mehu)
        self.kori.poista_tuote(mehu)
